...
describe("MappedSheetsLoader", () => {
  it("renames sheet column headers to model fields", async () => {
    ...
    const result = await new ProductLoader(source).loadMapped(
      "SHEET",
      "A1:C10",
    );
  ...
  it("calls loadWithHeaders with the correct sheetId and range", async () => {
    const source = makeSource([]);
    await new ProductLoader(source).loadMapped(
      "MY_SHEET_ID",
      "Products!A1:C500",
    );
    expect(source.loadWithHeaders).toHaveBeenCalledWith(
      "MY_SHEET_ID",
      "Products!A1:C500",
    );
  });
  ...
  it("reverseMap returns original column header keys", () => {
    ...
    const reversed = loader.reverseMap({
      id: "p1",
      name: "Widget",
      unitPrice: "9.99",
    });
