...
describe("SheetsLoader", () => {
  ...
  it("loads values and caches them", async () => {
    ...
    vi.spyOn(fetcher, "fetchValues").mockResolvedValue(
      fakeRows as unknown as string[][],
    );
  ...
  it("uses explicitAuthClient and skips getAuthClient", async () => {
    ...
    expect(fetcher.fetchValues).toHaveBeenCalledWith(
      fakeClient,
      "sheet1",
      "RANGE",
    );
  ...
  it("maps rows with mapper", async () => {
    ...
    const loader = new SheetsLoader();
    const mapped = await loader.loadAndMap("sheetA", "RANGE", (r) => ({
      k: r[0],
      v: r[1],
    }));
  ...
  it("wraps auth failure in AuthError", async () => {
    ...
    const loader = new SheetsLoader();
    await expect(loader.load("sheet1", "RANGE")).rejects.toBeInstanceOf(
      AuthError,
    );
  });
  ...
  it("rethrows SheetNotFoundError", async () => {
    vi.spyOn(auth, "getAuthClient").mockResolvedValue(null as unknown);
    vi.spyOn(fetcher, "fetchValues").mockRejectedValue(
